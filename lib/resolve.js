...
function resolve(options, done) {
  ...
  function resolveRecurse(dir, allowedVersion, done) {
    ...
    function filterFunction() {
      ...
      if (_.isNull(trueOptions.filter)) {
        return true;

      //call a callback function
      } else if (_.isFunction(trueOptions.filter)) {
        return trueOptions.filter(pkg);
        
      //If the use did not pass in null or a function, don't accept anything
      } else {
        return false;
      }
    }
